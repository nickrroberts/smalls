...
test("Ship can't be placed outside of Gameboard", () => {
    expect(board.placeShip("battleship", [1,9], "vert")).toBe("Invalid placement! Ship must be entirely on the game board.");
})

test("Ship cannot be placed on another ship occupying the same space", () => {
    expect(board.placeShip("submarine", [1,2], "horiz")).toBe("Invalid placement! Cannot place a ship on another ship.");
})

test("No more than one of each ship type can be placed on the board", () => {
    expect(board.placeShip("carrier", [1,2], "horiz")).toBe("Invalid! Can only place one of each ship type.");
})

test("Can't shoot at a spot you've already shot", () => {
    board.receiveAttack([3,4]);
    expect(board.receiveAttack([3,4])).toBe("Invalid move. You've already hit that spot.");
})
